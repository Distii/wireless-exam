...
\question{Describe a possible brute force attack to a WPA-personal setup (3 points)
    Explain how WPA3 enhances protection against offline dictionary attacks. What specific features of WPA3 contribute to this improved security? (3 points)
    Show the sequences of messages two stations exchange to complete the initial mutual authentication in WP3 using the Simultaneous Authentication of Equals (SAE) protocol.
    To describe the timeline, you can use a numbered list and state who sends what to who, or which computations a device does with some information:
    \begin{enumerate}
        \item STA sends frame XXX to AP
        \item AP sends frame YYY to broadcast
...
\question{Consider the curves below which show the Theoretical Maximum Throughout (TMT) in the 802.11g standard under various setups.
    Assume an STA has to send a file of 100MB (100*10^6 Bytes) directly to the AP, using UDP messages of 1000B (including all upper layer headers). What would be the minimum time to complete the transfer if the transmitter uses the 54Mbps (OFDM) modulation? (assume ideal conditions, UDP header is 8B, IP header is 20B, TCP header is 20B)
}
\begin{checkboxes}
    \choice About 0,3 seconds
    \choice About 3 seconds
    \CorrectChoice About 30 seconds
    \choice About 300 seconds
    \choice None of the other options.
\end{checkboxes}

\question{Consider the curves below which show the Theoretical Maximum Throughout (TMT) in the 802.11g standard under various setups.
    Assume an STA has to send a file of 100MB (100*10^6 Bytes) directly to the AP, using UDP messages of 1000B (including all upper layer headers). What would be the minimum time to complete the transfer if the transmitter uses the 24Mbps (OFDM) modulation? (assume ideal conditions, UDP header is 8B, IP header is 20B, TCP header is 20B)
}
\begin{checkboxes}
    \choice About 0,48 seconds
    \choice About 4.8 seconds
    \CorrectChoice About 48 seconds
    \choice About 480 seconds
    \choice None of the other options.
\end{checkboxes}